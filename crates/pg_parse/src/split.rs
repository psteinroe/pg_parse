...
/// let statements = pg_parse::split_with_parser(query).unwrap();
...
/// let result = pg_parse::split_with_parser(query);
/// let err = r#"syntax error at or near "this""#;
/// assert_eq!(result, Err(pg_parse::Error::Split(err.to_string())));
/// ```
...
/// let statements = pg_parse::split_with_scanner(query).unwrap();
